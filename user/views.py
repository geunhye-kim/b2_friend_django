...
def signup_view(request):
    if request.method == 'GET':
        ...
        if user_password != user_password2:
            ...
            if exist_user:
                return render(request, 'user/signup.html', {'error':'이미 이용중인 아이디입니다..'})  # 사용자가 존재하기 때문에 사용자를 저장하지 않고 회원가입 페이지를 다시 띄움
            else:
                UserModel.objects.create_user(username=user_name, password=user_password)
